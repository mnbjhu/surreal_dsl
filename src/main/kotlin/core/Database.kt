...
data class Database(val nameSpace: NameSpace, val name: String){
    ...
    suspend fun websocket(): DatabaseWebsocketConnection {
        val connection = client.webSocketSession("ws://${nameSpace.server.host}:${nameSpace.server.port}/rpc")
        return DatabaseWebsocketConnection(this@Database, connection)
    }

}

class DatabaseConnection(private val database: Database, val auth: Auth){
    suspend fun <T, U: ReturnType<T>>transaction(scope: TransactionScope.() -> U): T {
        ...
        val response = sendQuery(transaction.getQueryString())
        ...
        val r = surrealJson
            .decodeFromString(serializer, response.bodyAsText())
            .last() as ResultSet<T>
        return r.result
    }
